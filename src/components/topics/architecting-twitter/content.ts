...
const en = {
  ...
  timeline: {
    ...
    fanout: {
      ...
      realWorld: {
        title: 'Who uses what — and where celebrities break it',
        cols: { system: 'System', approach: 'Approach', note: 'Why / when it wins' },
        rows: [
          { system: 'Twitter / X', approach: 'Hybrid', note: 'Fanout-on-write for normal accounts; pull mega-accounts at read time and merge. Wins when most users are small but a few are enormous.' },
          { system: 'Instagram feed', approach: 'Pull-leaning + ranking', note: 'Assemble candidates at read time, then ML-rank. Wins when the feed is ranked, not purely chronological.' },
          { system: 'Threads', approach: 'Push + heavy caching', note: 'Precompute and cache aggressively for a chronological-first feed; wins on read latency at launch scale.' },
        ],
        celebrities:
          'The celebrity is the whole reason a single strategy fails. One account with 100M+ followers turns a pure fanout-on-write into 100M writes per tweet, so those accounts are excluded from write-time fanout and pulled in at read time instead. "Where is the celebrity cutoff?" is the follow-up interviewers love.',
      },
    },
  },
  ...
  blueprint: {
    ...
    legend: {
      ...
      async: 'Async / change-data-capture',
    },
    nodes: {
      cdn: 'CDN',
      client: 'Client',
      clientSub: 'Web · Mobile',
      lb: 'Load Balancer',
      gateway: 'API Gateway',
    ...
    edges: {
      ...
      read: 'read',
      cdc: 'CDC',
      fanoutWrite: 'fanout-on-write',
      fanoutRead: 'fanout-on-read',
      deliver: 'media delivery',
    },
    note: 'Read this map in three colours: the request path (client → gateway → services), the write side (tweet → MongoDB → queue → fanout → cache) and the read side (timeline → cache, pulled back to the CDN). Everything below just animates one slice of this picture at a time.',
  },

  estimations: {
    title: 'Back-of-the-envelope — sizing it live',
    subtitle:
      'Interviews are full of estimates. Drag the inputs and watch throughput, storage, cache, and machine counts recompute. These are order-of-magnitude numbers, not exact capacity plans.',
    inputs: {
      dau: { label: 'Daily active users', unit: 'M' },
      tweetsPerUser: { label: 'Tweets / active user / day', unit: '' },
      readsPerUser: { label: 'Timeline loads / user / day', unit: '' },
      mediaPct: { label: 'Tweets with media', unit: '%' },
    },
    assumptionsLabel: 'Assumptions',
    assumptions: [
      'Tweet record ≈ 1 KB (text + metadata + indexes)',
      'Average media object ≈ 2 MB',
      'Peak traffic ≈ 3× the daily average',
      'Timeline cache ≈ 800 tweet ids × 16 B per active user',
      'One app server ≈ 10k requests / second',
    ],
    peakLabel: 'peak',
    outputs: {
      tweetsPerDay: 'Tweets / day',
      peakWrites: 'Peak writes / s',
      peakReads: 'Peak reads / s',
      storagePerDay: 'New tweet storage / day',
      mediaPerYear: 'Media storage / year',
      cacheRam: 'Timeline cache RAM',
      servers: 'App servers (reads)',
    },
    note: 'Notice the shape: reads dwarf writes by ~100×, media dominates storage, and the timeline cache alone is terabytes of RAM. That imbalance is exactly why the design caches reads and pushes writes onto a queue.',
  },

  failures: {
    title: 'What happens when it breaks?',
    subtitle:
      'Staff-level answers name the blast radius. Pick a failure and see what degrades, what keeps serving, and how the system recovers.',
    statusLabels: { ok: 'Serving', degraded: 'Degraded', down: 'Down' },
    componentsLabel: 'Blast radius',
    components: [
      { id: 'writes', label: 'Write path' },
      { id: 'reads', label: 'Timeline reads' },
      { id: 'cache', label: 'Redis cache' },
      { id: 'content', label: 'MongoDB' },
      { id: 'events', label: 'Kafka / fanout' },
      { id: 'search', label: 'Elasticsearch' },
      { id: 'media', label: 'S3 / CDN' },
    ],
    impactLabel: 'Impact',
    ...
    scenarios: [
      {
        id: 'redis',
        label: 'Redis is down',
        impact: 'Timeline reads lose their fast path and fall back to rebuilding from MongoDB and the follow graph — much slower, and the origin stores take the extra load.',
        survives: 'Writes still persist to MongoDB; the source of truth is untouched. Nothing is lost.',
        recovery: 'Failover to a Redis replica; as feeds are re-requested the cache warms back up. Guard the origin with request coalescing so a cold cache does not stampede MongoDB.',
      },
      {
        id: 'mongo',
        label: 'MongoDB node lost',
        impact: 'A brief write blip on the affected shard while the replica set elects a new primary.',
        survives: 'Reads continue from secondaries; other shards are unaffected. No data loss with a healthy replica set.',
        recovery: 'Automatic primary election, then re-replicate onto a replacement node. This is why content is sharded and replicated, not single-homed.',
      },
      {
        id: 'kafka',
        label: 'Kafka consumers lag 5 min',
        impact: 'Fanout, search indexing, and notifications fall behind: timelines and search results go stale by minutes.',
        survives: 'The write path is unaffected — tweets still persist and events are durably queued, not dropped. Reads still work, just slightly stale.',
        recovery: 'Consumers catch up by draining the backlog (scale out consumer groups). Eventual consistency is the deliberate trade for a resilient write path.',
      },
      {
        id: 'region',
        label: 'A whole region goes down',
        impact: 'Elevated latency and some in-flight requests fail as traffic is shifted away from the failed region.',
        survives: 'Other regions keep serving from their replicas; durable stores (MongoDB, S3, Kafka) are cross-region replicated.',
        recovery: 'Shift traffic via DNS/global load balancing, promote replicas in a healthy region, and reconcile once the region returns. This is why multi-region and DR are first-class concerns.',
      },
    ],
    note: 'The pattern behind every row: the source of truth is protected and replicated, derived layers (cache, search, timelines) can lag or rebuild, and the queue absorbs downstream failure without blocking writes.',
  },
};
...
const ptBR: typeof en = {
  ...
  timeline: {
    ...
    fanout: {
      ...
      realWorld: {
        title: 'Quem usa o quê — e onde as celebridades quebram',
        cols: { system: 'Sistema', approach: 'Abordagem', note: 'Por que / quando ganha' },
        rows: [
          { system: 'Twitter / X', approach: 'Híbrido', note: 'Fanout-on-write para contas normais; megacontas puxadas na leitura e mescladas. Ganha quando a maioria é pequena mas alguns são enormes.' },
          { system: 'Feed do Instagram', approach: 'Pull + ranking', note: 'Monta candidatos na leitura e ranqueia com ML. Ganha quando o feed é ranqueado, não puramente cronológico.' },
          { system: 'Threads', approach: 'Push + cache pesado', note: 'Pré-computa e cacheia agressivamente para um feed cronológico; ganha em latência de leitura na escala de lançamento.' },
        ],
        celebrities:
          'A celebridade é a razão de uma estratégia única falhar. Uma conta com 100M+ seguidores transforma o fanout-on-write puro em 100M escritas por tweet, então essas contas são excluídas do fanout na escrita e puxadas na leitura. "Onde fica o corte de celebridade?" é o follow-up que os entrevistadores adoram.',
      },
    },
  },
  ...
  blueprint: {
    ...
    legend: {
      ...
      async: 'Assíncrono / change-data-capture',
    },
    nodes: {
      cdn: 'CDN',
      client: 'Cliente',
      ...
      timeline: 'Serviço de Timeline',
      fanout: 'Timeline Fanout',
      profile: 'Serviço de Perfil',
      auth: 'Auth & IAM',
      mq: 'Fila de Mensagens',
      tweetContent: 'Conteúdo de Tweet',
      media: 'Mídia',
      replies: 'Replies',
      es: 'Elasticsearch',
      timelineCache: 'Cache de Timeline',
      userData: 'Dados de Usuário',
      following: 'Dados de Follow',
    },
    edges: {
      https: 'HTTPS',
      write: 'escrita',
      ...
      fanoutWrite: 'fanout-on-write',
      fanoutRead: 'fanout-on-read',
      deliver: 'entrega de mídia',
    },
    note: 'Leia este mapa em três cores: o caminho de requisição (cliente → gateway → serviços), o lado de escrita (tweet → MongoDB → fila → fanout → cache) e o lado de leitura (timeline → cache, puxado de volta para a CDN). Tudo abaixo apenas anima uma fatia dessa imagem por vez.',
  },

  estimations: {
    title: 'Conta de padeiro — dimensionando ao vivo',
    subtitle:
      'Entrevistas são cheias de estimativas. Arraste as entradas e veja throughput, storage, cache e número de máquinas recalcularem. São números de ordem de grandeza, não um plano de capacidade exato.',
    inputs: {
      dau: { label: 'Usuários ativos por dia', unit: 'M' },
      tweetsPerUser: { label: 'Tweets / usuário ativo / dia', unit: '' },
      readsPerUser: { label: 'Cargas de timeline / usuário / dia', unit: '' },
      mediaPct: { label: 'Tweets com mídia', unit: '%' },
    },
    assumptionsLabel: 'Premissas',
    assumptions: [
      'Registro de tweet ≈ 1 KB (texto + metadados + índices)',
      'Objeto de mídia médio ≈ 2 MB',
      'Pico de tráfego ≈ 3× a média diária',
      'Cache de timeline ≈ 800 ids de tweet × 16 B por usuário ativo',
      'Um app server ≈ 10k requisições / segundo',
    ],
    peakLabel: 'pico',
    outputs: {
      tweetsPerDay: 'Tweets / dia',
      peakWrites: 'Escritas de pico / s',
      peakReads: 'Leituras de pico / s',
      storagePerDay: 'Novo storage de tweets / dia',
      mediaPerYear: 'Storage de mídia / ano',
      cacheRam: 'RAM do cache de timeline',
      servers: 'App servers (leitura)',
    },
    note: 'Repare no formato: leituras superam escritas em ~100×, mídia domina o storage e só o cache de timeline já são terabytes de RAM. Esse desequilíbrio é exatamente por que o design cacheia leituras e joga escritas numa fila.',
  },

  failures: {
    title: 'O que acontece quando quebra?',
    subtitle:
      'Respostas de nível Staff nomeiam o raio de impacto. Escolha uma falha e veja o que degrada, o que continua servindo e como o sistema recupera.',
    statusLabels: { ok: 'Servindo', degraded: 'Degradado', down: 'Fora' },
    componentsLabel: 'Raio de impacto',
    components: [
      { id: 'writes', label: 'Caminho de escrita' },
      { id: 'reads', label: 'Leituras de timeline' },
    ...
    scenarios: [
      {
        id: 'redis',
        label: 'Redis caiu',
        impact: 'As leituras de timeline perdem o caminho rápido e voltam a reconstruir a partir do MongoDB e do grafo de follow — bem mais lento, e os stores de origem levam a carga extra.',
        survives: 'As escritas continuam persistindo no MongoDB; a fonte da verdade fica intacta. Nada é perdido.',
        recovery: 'Failover para uma réplica do Redis; conforme os feeds são pedidos de novo, o cache reaquece. Proteja a origem com coalescência de requisições para um cache frio não estourar o MongoDB.',
      },
      {
        id: 'mongo',
        label: 'Nó do MongoDB perdido',
        impact: 'Um breve soluço de escrita no shard afetado enquanto o replica set elege um novo primário.',
        survives: 'As leituras continuam nos secundários; outros shards não são afetados. Sem perda de dados com um replica set saudável.',
        recovery: 'Eleição automática de primário e re-replicação num nó substituto. É por isso que o conteúdo é shardado e replicado, não single-homed.',
      },
      {
        id: 'kafka',
        label: 'Consumidores do Kafka atrasam 5 min',
        impact: 'Fanout, indexação de busca e notificações ficam para trás: timelines e resultados de busca ficam velhos por minutos.',
        survives: 'O caminho de escrita não é afetado — tweets ainda persistem e eventos ficam duravelmente enfileirados, não descartados. As leituras funcionam, só um pouco velhas.',
        recovery: 'Os consumidores se recuperam drenando o backlog (escale os consumer groups). Consistência eventual é a troca deliberada por um caminho de escrita resiliente.',
      },
      {
        id: 'region',
        label: 'Uma região inteira cai',
        impact: 'Latência elevada e algumas requisições em voo falham enquanto o tráfego é desviado da região que caiu.',
        survives: 'Outras regiões seguem servindo de suas réplicas; stores duráveis (MongoDB, S3, Kafka) são replicados entre regiões.',
        recovery: 'Desvie o tráfego via DNS/balanceamento global, promova réplicas numa região saudável e reconcilie quando a região voltar. É por isso que multi-region e DR são preocupações de primeira classe.',
      },
    ],
    note: 'O padrão por trás de cada linha: a fonte da verdade é protegida e replicada, camadas derivadas (cache, busca, timelines) podem atrasar ou reconstruir, e a fila absorve a falha downstream sem bloquear as escritas.',
  },
};
